...
#[tokio::test]
async fn test_ok_validate_create_output_subdirs() {
    require_fixture("basic.pdf");
...
async fn test_error_output_same_as_input() {
    ...
    let input_path = fixture_path("basic.pdf");
...
async fn test_error_no_clobber_with_existing_file() {
    ...
    assert!(
        result.is_err(),
        "Should fail with no-clobber and existing file"
    );
...
async fn test_error_invalid_page_range_format() {
    ...
    assert!(
        result.is_err(),
        "Should fail with invalid page range format"
    );
...
async fn test_error_all_files_fail_with_continue() {
    ...
    let config = Config {
        inputs: vec![temp1.path().to_path_buf(), temp2.path().to_path_buf()],
        output: output.to_path_buf(),
        dry_run: false,
        verbose: false,
        overwrite_mode: OverwriteMode::Force,
